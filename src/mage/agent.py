...
class TopAgent:
    def __init__(
        self,
        llm: LLM,
        rag_csv_path: str = "rtl_coder_dataset.csv",
        rag_k: int = 3,
    ):
        """
        Initialize TopAgent with optional RAG support.
        :param llm: an LLM instance
        :param rag_csv_path: path to RTL-Coder CSV corpus
        :param rag_k: number of examples to retrieve
        """
        self.llm = llm
        # ─── Vanilla RAG setup ──────────────────────────
        if rag_csv_path:
    ...
    def run_instance(self, spec: str) -> Tuple[bool, str]:
        assert self.tb_gen and self.rtl_gen and self.sim_reviewer and self.sim_judge and self.rtl_edit
        ...
        # Generate initial testbench & interface
        self.tb_gen.reset()
        self.tb_gen.set_golden_tb_path(self.golden_tb_path)
        if not self.golden_tb_path:
            logger.info("No golden testbench provided")
        ...
        # Simulation and potential edits
        tb_need_fix, rtl_need_fix = True, True
        sim_log = ""
        for i in range(self.sim_max_retry):
            is_sim_pass, sim_mismatch_cnt, sim_log = self.sim_reviewer.review()
            if is_sim_pass:
                tb_need_fix = rtl_need_fix = False
                break
            self.sim_judge.reset()
            tb_need_fix = self.sim_judge.chat(spec, sim_log, rtl_code, testbench)
            if tb_need_fix:
        ...
        candidates_info: List[Tuple[str, int, str]] = []
        if rtl_need_fix:
            assert sim_mismatch_cnt > 0, f"rtl_need_fix only when sim_mismatch_cnt>0"
            self.rtl_gen.reset()
            candidates = [
                self.rtl_gen.chat(
                    input_spec=spec,
            ...
            for i, (ok, code) in enumerate(candidates, 1):
                logger.info(f"Candidate generation: round {i}/{self.rtl_max_candidates}")
                if not ok:
                    continue
                self.write_output(code, "rtl.sv")
                simp, cnt, slog = self.sim_reviewer.review()
                if simp:
                    rtl_code, sim_mismatch_cnt, sim_log = code, cnt, slog
                    rtl_need_fix = False
                    break
                candidates_info.append((code, cnt, slog))
        ...
        if rtl_need_fix:
            for i in range(self.rtl_selected_candidates):
                code, cnt, slog = unique[i % len(unique)]
                with open(f"{self.output_dir_per_run}/rtl.sv", "w") as f:
                    f.write(code)
                self.rtl_edit.reset()
                simp, code = self.rtl_edit.chat(
                    spec=spec,
                    output_dir_per_run=self.output_dir_per_run,
                    sim_failed_log=slog,
                    sim_mismatch_cnt=cnt,
                )
                if simp:
                    rtl_code = code
                    rtl_need_fix = False
    ...
    def run_instance_ablation(self, spec: str) -> Tuple[bool, str]:
        ...
        self.rtl_gen.reset()
        logger.info(spec)
        ok, code = self.rtl_gen.ablation_chat(
            input_spec=spec,
            rtl_path=os.path.join(self.output_dir_per_run, "rtl.sv"),
        )
        self.write_output(code, "rtl.sv")
    ...
    def _run(self, spec: str) -> Tuple[bool, str]:
        try:
            tag = f"{self.output_dir_per_run}/properly_finished.tag"
            ...
            result = (
                self.run_instance(spec)
                if not self.is_ablation else 
                self.run_instance_ablation(spec)
            )
            self.token_counter.log_token_stats()
            ...
            result = False, f"Exception: {exc_val}"
        return result
    ...
    def run(
        ...
        self.golden_tb_path = golden_tb_path
        self.golden_rtl_blackbox_path = golden_rtl_blackbox_path
        log_dir = f"{self.log_path}/{benchmark_type_name}_{task_id}"
        out_dir = f"{self.output_path}/{benchmark_type_name}_{task_id}"
        os.makedirs(self.output_path, exist_ok=True)
        os.makedirs(out_dir, exist_ok=True)
        set_log_dir(log_dir)
        self.output_dir_per_run = out_dir

        if self.redirect_log:
            with open(f"{log_dir}/mage_rtl.log", "w") as f:
                sys.stdout = f
                sys.stderr = f
                result = self._run(spec)
            sys.stdout = sys.__stdout__
        ...
        if self.redirect_log:
            with open(f"{log_dir}/mage_rtl.log", "r") as f:
                content = f.read()
            clean = re.sub(r"\x1B\[[0-?]*[ -/]*[@-~]", "", content)
            with open(f"{log_dir}/mage_rtl_rich_free.log", "w") as f:
                f.write(clean)
